/*
 * Fichero:   		matadd_16b.cpp
 * Autor: 		    Diego García Aranda 820506
 * Fecha entrega:	03/06/2024
 * Comentarios:     Fichero que realiza una suma de matrices con saturación para enteros
 *                  sin signo de 16 bits. Se ejecutan dos versiones: con y sin 
...
int main() {
    ...
    for (uint32_t i = 0; i < N*N; i++) {
        uint16_t res = data1[i] + data2[i];
        if (res < data1[i]) res = UINT16_MAX;
        resul_base[i] = res;
    }
    ...
    for (uint32_t i = 0; i < N*N; i+= 2) {
        asm volatile (
            "lw t3, 0(%0)\n\t"       // Cargar los valores de 16 bits de data1[i:i+1] en t3
            "lw t4, 0(%1)\n\t"       // Cargar los valores de 16 bits de data2[i:i+1] en t4
            "add16 t4, t3, t4\n\t"   // t4 almacena la suma de los valores de 16 bits de t3 y t4 
            "sltu16 t3, t4, t3\n\t"  // t3 almacena los valores de 16 bits cuya suma ha producido un desbordamiento
            "or t3, t3, t4\n\t"      // t3 almacena la suma de los valores de 16 bits de de data1[i:i+1] y data2[i:i+1] con 
                                     // saturación
            "sw t3, 0(%2);"          // Escribir t3 en resul_simd[i:i+1]
